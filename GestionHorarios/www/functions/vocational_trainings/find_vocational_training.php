...
try {
    ...
    if (isset($_POST["txtFindVocationalTraining"]) && !empty($_POST["txtFindVocationalTraining"])) {
        ...
        $query = $pdo->prepare("SELECT * FROM vocational_trainings WHERE course_code LIKE LOWER(?) OR acronym LIKE LOWER(?) OR course_name LIKE LOWER(?) OR modality LIKE LOWER(?) OR type LIKE LOWER(?)");
