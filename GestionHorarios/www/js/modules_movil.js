  ...
  fetch("../functions/modules/get_modules.php", {
    ...
    .then((data) => {
      ...
      selects.forEach((select) => {
        ...
        const assignedModId =
          data.assignedModules && data.assignedModules[sessionId]
            ? data.assignedModules[sessionId]
            : "";
...
function onSelectChange(event) {
  ...
  if (newValue) {
    ...
    if (usage[newValue] > maxSessions) {
      alert(
        `Has superado el número máximo de sesiones para el módulo: ${modulesMap[newValue].name} (máximo: ${maxSessions})`
      );
